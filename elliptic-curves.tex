...
\begin{enumerate}
    ...
    \item[!5.] Prove that for every positive integer $N\equiv5$ mod 8, the
        elliptic curve
        \begin{equation*}
            y^2 = x^3-N^2x
        \end{equation*}
...
\begin{enumerate}
    ...
    \item[3.] Let $E/K$ be an elliptic curve over a field of characteristic
        zero. Prove that
        \begin{equation*}
            E(\bar K)[n] \cong \Z/n\Z\oplus\Z/n\Z.
        \end{equation*}
        ...
        \begin{proof}[Solution]
            ...
            $E(\bar F)[n]\cong E(\C)[n]$. Hence
            \begin{equation*}
                E(\bar K)[n] = E(\bar F)[n] \cong E(\C)[n]
                    \cong \Z/n\Z\oplus\Z/n\Z
            \end{equation*}
...
\begin{proof}
    ...
    $E_\C\cong E_\Lambda$ for some $\Lambda$, and by Proposition 2(ii) this
...
\begin{lemma}
    ...
    \begin{equation*}
        \{p\in\A:h(p)<c\}
    \end{equation*}
    is finite for each $c>0$. Then $\A\cong\Z^{\oplus n}$ for some $n\in\N$ or
    $\A\cong\Z^{\oplus\N}$.
\end{lemma}

\begin{theorem}
    Let $E/\Q$ be an elliptic curve. Then $E(\Q)\cong\Delta\times\Z^n$ or
...
\begin{proof}
    ...
    $Q=(x_0,y_0)$ then
    \begin{equation*}
        \sigma(x_0)=x_0\text{ mod $\p$},\quad\sigma(y_0)=y_0\text{ mod $\q$}
    \end{equation*}
...
\begin{enumerate}
    \item[+1.] Let $E/\Q$ be the elliptic curve given by $y^2+y=x^3-x^2$. Show
        ...
        \begin{proof}[Solution]
            ...
            $(x-7)-(x-8)=1\in(\F_{11}^\times)^2$. Now $E(\Q)[2]=\{\O\}$, since
...
\begin{lemma}
    ...
    \begin{equation*}
        [n](T)
            = \underbrace{T\oplus_\calF\cdots\oplus_\calF T}_{\text{$n$ times}}.
    \end{equation*}
...
\begin{corollary}[Theorem 37]
    If $E/\Q_p$ is an elliptic curve, then $E_1(\Q_p)$ has no points of finite
    order, except possibly those of order 2 when $p=2$.
\end{corollary}
...
\begin{lemma}
    Let $E/K$ be an elliptic curve with (for simplicity) $K\subseteq\C$, given
    by
    \begin{equation*}
        y^2=(x-\alpha)(x-\beta)(x-\gamma)\quad\alpha,\beta,\gamma\in K.
    \end{equation*}
    For $P\in E(K)$ write $\frac{1}{2}P\in E(\bar K)$ for some point with
    $\frac{1}{2}P\oplus\frac{1}{2}P=P$.
    \begin{enumerate}[label=(\roman*)]
        \item $K(\frac{1}{2}P)/K$ is Galois with
            $\Gal(K(\frac{1}{2}P)/K)\le C_2\times C_2$.

        \item The map
            \begin{equation*}
                \phi_P:\Gal(\bar K/K)\to E(K)[2];\quad
                \phi_P(g)=\textstyle g(\frac{1}{2}P)\ominus\frac{1}{2}P
            \end{equation*}
            is a well-defined homomorphism with kernel
            $\Gal(\bar K/K(\frac{1}{2}P))$.

        \item The map
            \begin{equation*}
                \phi:E(K)/2E(K)\to\Hom_{\cts}(\Gal(\bar K/K),E(K)[2]);\quad
                P\mapsto\phi_P
            \end{equation*}
...
\begin{remark}
    A homomorphism $\phi:\Gal(\bar K/K)\to G$ for a finite group $G$ is
    continuous if it comes from a finite Galois extension, i.e. there is $F/K$
    finite and Galois with $\tilde\phi:\Gal(F/K)\to G$ such that $\phi$ is the
    composition $\Gal(\bar K/K)\to\Gal(F/K)\to G$. We say $\phi$ factors through
    $F/K$.
\end{remark}

\begin{proof}
    \begin{enumerate}[label=(\roman*)]
        \item By Lemma 25, since
            $E(K)[2]=\{\O,(0,\alpha),(0,\beta),(0,\gamma)\}$.
        \item See the proof of Lemma 25(iii).
        \item See Sheet 4, Exercise 3. Note that $\phi_P$ is continuous by (ii).
    \end{enumerate}
\end{proof}

\begin{remark}
    This is a refinement of our 16-to-1 map $P\mapsto K(\frac{1}{2}P)$;
    $P\mapsto\phi_P$ is now injective, respects addition, and recovers
    $K(\frac{1}{2}P)$ as the fixed field of $\ker\phi_P$.
\end{remark}

\begin{lemma}
    Let $E/K$ be an elliptic curve with $K\subseteq\C$, given by
    \begin{equation*}
        y^2=(x-\alpha)(x-\beta)(x-\gamma)\quad\alpha,\beta,\gamma\in K.
    \end{equation*}
    \begin{enumerate}[label=(\roman*)]
        \item We have a map
            \begin{equation*}
                \eta:\Hom_\cts(\Gal(\bar K/K),E(K)[2])
                    \to\frac{K^\times}{{K^\times}^2}
                        \times\frac{K^\times}{{K^\times}^2}
                        \times\frac{K^\times}{{K^\times}^2}; \quad
                \psi
                    \mapsto(\psi_\alpha,\psi_\beta,\psi_\gamma)
            \end{equation*}
            where $\psi(g)\in\{\O,(\alpha,0)\}$ iff
            $g\in\Gal(\bar K/K(\sqrt{\psi_\alpha}))$, and similar for
            $\beta,\gamma$. Then $\eta$ is an injective homomorphism; an
            isomorphism onto the subgroup of triples $a,b,c$ with
            $abc\in{K^\times}^2$.
            \begin{equation*}
                \begin{tikzcd}
                    & &K(\frac{1}{2}P) \ar[dl,dash] \ar[d,dash] \ar[dr,dash] & \\
                    &K(\sqrt{\psi_\alpha}) \ar[dr,dash]
                    &K(\sqrt{\psi_\beta}) \ar[d,dash]
                    &K(\sqrt{\psi_\gamma}) \ar[dl,dash] \\
                    & &K &
                \end{tikzcd}
            \end{equation*}

        \item If $P=(x_0,y_0)\in E(K)$ then
            $\eta(\phi_P)=(x_0-\alpha,x_0-\beta,x_0-\gamma)$ unless
            $x_0=\alpha$, in which case the first entry is
            $(x_0-\beta)(x_0-\gamma)$, and similar for $\beta,\gamma$.
    \end{enumerate}
\end{lemma}

\begin{remark}.
    (i) simply records the subfields of $K(\frac{1}{2}P)$ associating each
    quadratic to a specific 2-torsion point. (ii) says that these quadratics are
    just $K(\sqrt{x_0-\alpha})$, $K(\sqrt{x_0-\beta})$, $K(\sqrt{x_0-\gamma})$.
    Keeping this extra structure preserves the group structure on $E(K)$.
\end{remark}

\begin{proof}
    \begin{enumerate}[label=(\roman*)]
        \item $\Hom_\cts(\Gal(\bar K/K),C_2)\cong K^\times/{K^\times}^2$ via
            $\psi\mapsto d$ for $\ker\psi=\Gal(\bar K/K(\sqrt{d}))$. This is a
            homomorphism since if $\ker\psi_i=\Gal(\bar K/K(\sqrt{d_i}))$ for
            $i=1,2$ then
            $\ker(\psi_1\psi_2)=\Gal(\bar K/K(\sqrt{d_1}\sqrt{d_2}))$. Now
            apply to $E(K)[2]\cong C_2\times C_2$ to get
            $K^\times/{K^\times}^2\times K^\times/{K^\times}^2\times K^\times/{K^\times}^2$.
            Recording the third homomorphism gives the required map $\eta$.

        \item (Sketch) If $E:y^2=x^3+Ax^2+Bx$, then for $Q=(x_0,y_0)$ we have
            \begin{equation*}
                2Q = \left(\left(\frac{x_0-B}{2y_0}\right)^2,\ldots\right).
            \end{equation*}
            Hence if $2Q=P=(x_1,y_1)$ then $K(\frac{1}{2}P)$ contains
            $\sqrt{x_1}$, so if $E:y^2=(x-\alpha)(x-\beta)(x-\gamma)$ with
            $P=(x_2,y_2)$ then
            \begin{equation*}
                K({\textstyle\frac{1}{2}P})\supseteq
                K(\sqrt{x_2-\alpha},\sqrt{x_2-\beta},\sqrt{x_2-\gamma}).
            \end{equation*}
            Using $\alpha,\beta,\gamma$ as variables, and that
            $\Gal(K(\frac{1}{2}P)/K)\le C_2\times C_2$, we can deduce that
            \begin{equation*}
                K({\textstyle\frac{1}{2}P})
                    = K(\sqrt{x_2-\alpha},\sqrt{x_2-\beta},\sqrt{x_2-\gamma})
            \end{equation*}
            and no larger. Keep track of the Galois action to identify
            subfields for the final result.
    \end{enumerate}
\end{proof}

\begin{example}
    Consider $E:y^2=x(x-1)(x+1)$. Recall for $P\in E(\Q)$ that
    $\Q(\frac{1}{2}P)/\Q$ only ramifies at $p=2$, so
    $\Q(\frac{1}{2}P)\subseteq\Q(\sqrt2,i)$. Now
    $P=(x_0,y_0)\mapsto(x_0,x_0-1,x_0+1)\in(\Q^\times/{\Q^\times}^2)^3$ is a
    homomorphism giving the quadratic subfields of $\Q(\frac{1}{2}P)$. Hence
    $x_0,x_0-1,x_0+1$ are given up to squares by elements of $\{\pm1,\pm2\}$.
    We go through the possibilities:
    \begin{equation*}
        \begin{array}{|c|c|c|c|}
            \hline
            x_0 & x_0-1 & x_0+1 & \text{$\in$ image?} \\
            \hline
            +1 & +1 & +1 & \text{yes; 1)} \\
            +1 & -1 & -1 & \text{no; 2)} \\
            +1 & +2 & +2 & \text{yes; 1)} \\
    ...
    \end{equation*}
    \begin{enumerate}[label=\arabic*)]
        \item We have the 2-torsion points $(0,0),(1,0),(-1,0),\O\in E(\Q)$.
        \item We must have $x_0+1>0$, and $x_0(x_0-1)>0$.

        \item We prove directly that the triple $(2,1,2)$ cannot occur. If
            $x_0=2a^2$, $x_0-1=b^2$ and $x_0+1=2c^2$ with $a,b,c\in\Q^\times$,
            take a denominator $z\in\Z$ such that $az\in\Z$ and $(az,z)=1$. Then
            $2(az)^2-z^2=(bz)^2$ and $2(az)^2+z^2=2(cz)^2$, so
            $A=az,B=bz,C=cz$ are integers satisfying $2A^2-z^2=B^2$,
            $2A^2+z^2=2C^2$, and $(A,z)=1$.
            \begin{itemize}
                \item If $A$ is even then $z$ is odd, so $B^2\equiv-1$ mod 8,
                    which is impossible.

                \item If $A$ is odd, then $A^2\equiv1$ mod 8. If $z$ is also odd
                    then $2C^2\equiv3$ mod 8, which is impossible, and if $z$
                    is even then $B^2\equiv2$ or 6 mod 8, which is also
                    impossible.
            \end{itemize}

        \item As the map is a homomorphism, the image is a subgroup.
    \end{enumerate}
    Hence $\#E(\Q)/2E(\Q)=4$ so $\rk E=0$.
\end{example}

\begin{theorem}[Complete 2-descent]
    Let $K$ be a field of characteristic 0, and $E/K$ an elliptic curve given by
    \begin{equation*}
        E:y^2 = (x-\alpha)(x-\beta)(x-\gamma),\quad \alpha,\beta,\gamma\in K.
    \end{equation*}
    \begin{enumerate}[label=(\roman*)]
        \item The map $P\mapsto(x_0-\alpha,x_0-\beta,x_0-\gamma)$, replacing
            terms with the product of the other two if they vanish, and letting
            $\O\mapsto(1,1,1)$, is an injective homomorphism
            $E(K)/2E(K)\to(K^\times/{K^\times}^2)^3$.

        \item The triples $(a,b,c)$ that lie in the image satisfy
            $abc\in{K^\times}^2$. Either they are in the image of $E(K)[2]$, or
            \begin{equation*}
                cz_3^2-\alpha+\gamma = az_1^2,\quad cz_3^2-\beta+\gamma=bz_2^2
            \end{equation*}
            is soluble with $z_i\in K^\times$, in which case
            $P=(az_1^2+\alpha,\sqrt{abc}z_1z_2z_3)$ maps to $(a,b,c)$.

        \item If $K$ is a number field, and $(a,b,c)$ is in the image, then
            $K(\sqrt a,\sqrt b,\sqrt c)/K$ only ramifies at primes dividing
            $2(\alpha-\beta)(\beta-\gamma)(\alpha-\gamma)$. If $K=\Q$, then
            taking $a,b,c\in\Z$ square-free we get that $a,b,c$ only have
            prime factors $p\mid2(\alpha-\beta)(\beta-\gamma)(\alpha-\gamma)$,
            assuming $\alpha,\beta,\gamma\in\Z$.
    \end{enumerate}
\end{theorem}
...
\begin{proof}
    \begin{enumerate}[label=(\roman*)]
        \item Lemma 50.
        \item Solve $x_0-\alpha=az_1^2$, $x_0-\beta=bz_2^2$,
            $x_0-\gamma=cz_3^2$.
        \item Lemma 26.
    \end{enumerate}
\end{proof}
...
\begin{proposition}
    Suppose $E/\Q_p$ is an elliptic curve, given by
    \begin{equation*}
        E:y^2=(x-\alpha)(x-\beta)(x-\gamma),\quad\alpha,\beta,\gamma\in\Z_p.
    \end{equation*}
    \begin{enumerate}[label=(\roman*)]
        \item If $p\ne2$ then $\#E(\Q_p)/2E(\Q_p)=4$.
        \item If $p\nmid2\Delta_E$ then $\Q_p(\frac{1}{2}P)/\Q_p$ is unramified
            for $P\in E(\Q_p)$.
        \item If $p\nmid2\Delta_E$ then $(a,b,c)$ lies in the image of
            $E(\Q_p)/2E(\Q_p)$ iff $\ord_p(a),\ord_p(b),\ord_p(c)$ are all even
            and $abc\in{\Q_p^\times}^2$.
    \end{enumerate}
\end{proposition}

\begin{remark}
    (iii) says that only $p$ dividing $2\Delta_E$ give interesting constraints
    on the triples $(a,b,c)$.
\end{remark}
...
\begin{proof}
    \begin{enumerate}[label=(\roman*)]
        \item Consider
            \begin{equation*}
                \begin{tikzcd}
                    &0 \ar[r] &E_1(\Q_p) \ar[d,"\times2"] \ar[r]
                    &E(\Q_p) \ar[d,"\times2"] \ar[r]
                    &E(\Q_p)/E_1(\Q_p) \ar[d,"\times2"] \ar[r] &0 \\
                    &0 \ar[r] &E_1(\Q_p) \ar[r]
                    &E(\Q_p) \ar[r]
                    &E(\Q_p)/E_1(\Q_p) \ar[r] &0.
                \end{tikzcd}
            \end{equation*}
            If $K_1,K_2,K_3,C_1,C_2,C_3$ are the kernels and cokernels, we have the
            snake lemma:
            \begin{equation*}
                0\to K_1\to K_2\to K_3\to C_1\to C_2\to C_3\to 0.
            \end{equation*}
            By Corollary 46 the map $E_1(\Q_p)\xrightarrow{\times2}E_1(\Q_p)$ is an
            isomorphism, so $K_1=C_1=0$. Therefore
            \begin{equation*}
                \#E(\Q_p)/2E(\Q_p) = \#C_2 = \#C_3 = \#K_3 = \#K_2
                    = \#E(\Q_p)[2] = 4.
            \end{equation*}

        \item Lemma 26.
        \item Exercise (use (i) and (ii)).
    \end{enumerate}
\end{proof}

\begin{example}
    Consider $E:y^2=x(x-5)(x+5)$, with $\Delta_E=-2^65^6$, and recall the map
    \begin{align*}
        E(\Q)/2E(\Q) &\to (\Q^\times/{\Q^\times}^2)^3 \\
        P=(x_0,y_0) &\mapsto (x_0,x_0-5,x_0+5).
    \end{align*}
    Possible triples $(a,b,c)$ in the image have
    $a,b,c\in\{\pm1,\pm2,\pm5,\pm10\}$ and $abc=1$ up to squares.
    \begin{itemize}
        \item Over $\R$: The image of $E(\R)$ is $\{(+,+,+),(-,-,+)\}$.

        \item Over $\Q_5$: We have representatives for
            $\Q_5^\times/{\Q_5^\times}^2$ given by $1,2,5,10$. Note that
            $-1\in{\Q_5^\times}^2$. We know there are only 4 triples coming
            from $E(\Q_5)$ by Proposition 52(i). These must be $(1,1,1)$,
            $(1,5,5)$, $(5,2,10)$, $(5,10,2)$ from the 2-torsion points.
    \end{itemize}
    Combining this information we deduce that $P\in E(\Q)$ can only have image
    in
    \begin{equation*}
        \{(1,1,1),(-1,-1,1),(1,5,5),(-1,-5,5),(5,2,10),(-5,-2,10),(5,10,2),
            (-5,-10,2)\}.
    \end{equation*}
    Now $\O\mapsto(1,1,1)$, $(0,0)\mapsto(-1,-5,5)$, $(5,0)\mapsto(5,2,10)$,
    $(-5,0)\mapsto(-5,-10,2)$, and we have the point $P=(-4,6)\mapsto(-1,-1,1)$,
    so the image (which is a subgroup) must be the whole of this set. From this
    we deduce that $\rk E(\Q)=1$.
\end{example}

\subsection*{Exercises}

\begin{enumerate}
    \item[+1.] Compute the rank of $E:y^2=x(x+3)(x-6)$ over $\Q$.
        (\textit{Hint: $(-2,4)\in E(\Q)$.})

        \begin{proof}[Solution]
            The discriminant of the cubic is $((-3)(6)(9))^2$, so in Theorem 51
            the extensions only ramify at 2 or 3. Hence the image of the
            injective homomorphism
            $E(\Q)/2E(\Q)\to\bigl(\Q^\times/{\Q^\times}^2\bigr)^3$;
            $P=(x_0,y_0)\mapsto(x_0,x_0+3,x_0-6)$ consists of triples $(a,b,c)$
            with $a,b,c\in\{\pm1,\pm2,\pm3,\pm6\}$.
            \begin{itemize}
                \item Over $\R$, the possible triples are $(-,+,-)$ and
                    $(+,+,+)$.

                \item Over $\Q_3$, a complete set of representatives for
                    $\Q_3^\times/{\Q_3^\times}^2$ is $\{1,2,3,6\}$, and the
                    kernel of the map from
                    $\{\pm1,\pm2,\pm3,\pm6\}\le\Q^\times/{\Q^\times}^2$ to
                    $\Q_3^\times/{\Q_3^\times}^2$ is generated by $-2$. By
                    Proposition 52 there are only 4 triples coming from
                    $E(\Q_3)$, which must be the images of the 2-torsion points:
                    \begin{align*}
                        (1,1,1) &\sim_{{\Q_3^\times}^2} (1,1,1), &
                        (-2,3,-6) &\sim_{{\Q_3^\times}^2} (1,3,3), \\
                        (-3,2,-1) &\sim_{{\Q_3^\times}^2} (6,2,3), &
                        (6,1,6) &\sim_{{\Q_3^\times}^2} (6,1,6).
                    \end{align*}
            \end{itemize}
            Hence the image of $E(\Q)$ is contained in
            \begin{align*}
                \{
                    &(1,1,1),(6,3,3),(6,2,2),(6,1,6), \\
                    &(-2,1,-2),(-3,3,-6),(-3,2,-4),(-3,1,-3)
                \}.
            \end{align*}
            Now $(-2,4)\in E(\Q)$ maps to $(-2,1,-2)$, so the image has order a
            power of 2 greater than 5 and at most 8. The only such power of 2 is
            8, so $|E(\Q)/2E(\Q)|=8=2^{2+1}$ and $\rk E(\Q)=1$.
        \end{proof}

    \item[+2.] Compute the rank of $E:y^2=x^3-49x$ over $\Q$.
        (\textit{Hint: $(25,120)\in E(\Q)$.})

        \begin{proof}[Solution]
            The discriminant of the cubic is $((7)(-7)(14))^2$, so in Theorem 51
            the extensions only ramify at 2 or 7. Hence the image of the
            injective homomorphism
            $E(\Q)/2E(\Q)\to\bigl(\Q^\times/{\Q^\times}^2\bigr)^3$;
            $P=(x_0,y_0)\mapsto(x_0,x_0+3,x_0-6)$ consists of triples $(a,b,c)$
            with $a,b,c\in\{\pm1,\pm2,\pm7,\pm14\}$. Over $\Q_7$, a complete set
            of representatives for $\Q_7^\times/{\Q_7^\times}^2$ is
            $\{1,3,7,21\}$, and the kernel of the map from
            $\{\pm1,\pm2,\pm7,\pm14\}\le\Q^\times/{\Q^\times}^2$ to
            $\Q_7^\times/{\Q_7^\times}^2$ is generated by 2. By Proposition 52
            there are only 4 triples coming from $E(\Q_7)$, which must be the
            images of the 2-torsion points:
            \begin{align*}
                (1,1,1) &\sim_{{\Q_7^\times}^2} (1,1,1), &
                (-1,-7,7) &\sim_{{\Q_7^\times}^2} (3,21,7), \\
                (7,2,14) &\sim_{{\Q_7^\times}^2} (7,1,7), &
                (-7,-14,2) &\sim_{{\Q_7^\times}^2} (21,21,1).
            \end{align*}
            Hence the image of $E(\Q)$ consists of these, and triples obtained
            from these by multiplying two coordinates by 2. Triples $(a,b,c)$
            not coming from 2-torsion have some $z_1,z_2,z_3\in\Q^\times$
            satisfying
            \begin{equation*}
                cz_3^2-7=az_1^2, \quad cz_3^2-7-7=bz_2^2
            \end{equation*}
            by Theorem 51. If $N$ is a common denominator for $z_3$ and $z_1$,
            say $A=z_3N$ and $B=z_1N$, with $A,B,N$ having no common factor. If
            $a$ is even then $N$ must be odd: otherwise $A$ is even, so
            $4\mid2B^2$, and $2\mid A,B,N$. Then from $7N^2=aB^2-b(z_2N)^2$ we
            see that $a$ and $b$ cannot both be even, as multiplying a rational
            square by an integer of 2-adic valuation 1 cannot give an odd
            integer. Combining this with the observations over $\Q_7$, we see
            that the image must be contained in
            \begin{align*}
                \{
                    &(1,1,1),(-1,-7,7),(7,2,14),(-7,-14,2) \\
                    &(2,1,2),(-2,-7,14),(14,1,14),(-14,-7,2) \\
                    &(1,2,2),(-1,-14,14),(7,1,7),(-7,-7,2)
                \}.
            \end{align*}
            Now $(25,120)\in E(\Q)$ maps to $(1,2,2)$, so the image has order a
            power of 2 greater than 5 and at most 12. The only such power of 2
            is 8, so $|E(\Q)/2E(\Q)|=8=2^{2+1}$ and $\rk E(\Q)=1$.
        \end{proof}

    \item[3.] Let $E/\Q_2$ be an elliptic curve with $E(\Q_2)[2]=C_2\times C_2$.
        Show that $|E(\Q_2)/2E(\Q_2)|=8$.

    \item[4.] Let $K$ be a field of characteristic 0 that contains the
        $p^{\text{th}}$ roots of unity, for some prime $p$. Show that
        $\Hom_\cts(\Gal(\bar K/K),\Z/p\Z)\cong K^\times/{K^\times}^p$.

    \item[!5.] Let $E/\Q$ be an elliptic curve given by
        $E:y^2=(x-\alpha)(x-\beta)(x-\gamma)$ for some
        $\alpha,\beta,\gamma\in\Z$. Let $S$ denote the group of those triples
        $(a,b,c)\in\Q^\times/{\Q^\times}^2
        \times\Q^\times/{\Q^\times}^2\times\Q^\times/{\Q^\times}^2$
        with $abc\in{\Q^\times}^2$, which (when working modulo ${\R^\times}^2$
        or ${\Q_p^\times}^2$) lie in the image of $E(\R)/2E(\R)$ and
        $E(\Q_p)/2E(\Q_p)$ for every prime $p$. Prove that $|S|=2^{\rk(E/\Q)+n}$
...
\begin{definition}
    Let $E/K$ be an elliptic curve, and $\ell$ a prime. The \emph{$\ell$-adic
    Tate module} $T_\ell E$ is the projective limit
    \begin{equation*}
        T_\ell E = \varprojlim_nE(\bar K)[\ell^n]
    \end{equation*}
    with respect to the multipication-by-$\ell$ maps, i.e. the topological group
    of sequences $P_1,P_2,\ldots$ such that $P_n\in E(\bar K)[\ell^n]$ and
    $\ell P_{n+1}=P_n$.
\end{definition}

\begin{lemma}
    If $\ch K=0$, then $T_\ell E\cong\Z_\ell\oplus\Z_\ell$ as a topological
    group.
\end{lemma}

\begin{proof}
    We have $E(\bar K)[\ell^n]\cong\Z/\ell^n\Z\times\Z/\ell^n\Z$ by Lemma 25 and
    the Lefschetz principle. Taking projective limits gives the result.
\end{proof}

\begin{numberremark}
    The main purpose of using $T_\ell E$ is to keep track of the actions of
    $G_K=\Gal(\bar K/K)$ on each $E(\bar K)[\ell^n]$ simultaneously. If
    $g\in G_K$ and $P_1,Q_1\in E(\bar K)[\ell]$ is a basis then we have
    $g(P_1)=a_1P_1\oplus b_1Q_1$, $g(Q_1)=c_1P_1\oplus d_1Q_1$ for some
    $0\le a_1,b_1,c_1,d_1<\ell$, i.e.
    \begin{equation*}
        g \begin{pmatrix}
            P_1 \\ Q_1
        ...
        \end{pmatrix}.
    \end{equation*}
    If $P_2,Q_2\in E(\bar K)[\ell^2]$ is a basis with $\ell P_1=P_2$,
    $\ell Q_1=Q_2$ then similarly we have $0\le a_2,b_2,c_2,d_2<\ell$ satisfying
    \begin{align*}
        g(P_2) &= (a_1+\ell a_2)P_2\oplus(b_1+\ell b_2)Q_2 \\
        g(Q_2) &= (c_1+\ell c_2)P_2\oplus(d_1+\ell d_2)Q_2,
    \end{align*}
    since multiplying by $\ell$ must give the previous expression. Continuing
    inductively we find $P_n,Q_n\in E(\bar K)[\ell^n]$ such that
    $P_{n-1}=\ell P_n$, $Q_{n-1}=\ell Q_n$ and
    \begin{align*}
        g(P_n) &= (a_1+\ell a_2+\ell^2a_3+\cdots+\ell^{n-1}a_n)P_n
            \oplus(b_1+\ell b_2+\ell^2b_3+\cdots+\ell^{n-1}b_n)Q_n \\
        g(Q_n) &= (c_1+\ell c_2+\ell^2c_3+\cdots+\ell^{n-1}c_n)P_n
            \oplus(d_1+\ell d_2+\ell^2d_3+\cdots+\ell^{n-1}d_n)Q_n.
    \end{align*}
    Hence $g$ is described by an element of $\GL_2(\Z_\ell)$, and the action on
    $E(\bar K)[\ell^n]$ is simply given by reducing this matrix mod $\ell^n$.
\end{numberremark}

\begin{lemma}
    Let $\tilde E/\F_{p^k}$ be an elliptic curve over a finite field, and $E/K$
    an elliptic curve over a local field with good reduction, $\F_K=\F_{p^k}$,
    and reduced curve $\tilde E$. Let $\ell\ne p$ be a prime.
    \begin{enumerate}[label=(\roman*)]
        \item The reduction map gives an isomorphism
            $E(K)[\ell^n]\xrightarrow\sim\tilde E(\F_{p^k})[\ell^n]$.

        \item $\tilde E(\bar\F_{p^k})[\ell^n]\cong\Z/\ell^n\Z\times\Z/\ell^n\Z$.

        \item $T_\ell\tilde E\cong\Z_\ell\times\Z_\ell$.

        \item The extension $K(E[\ell^n])/K$ is unramified for all $n$.
    \end{enumerate}
\end{lemma}

\begin{proof}
    \begin{enumerate}[label=(\roman*)]
        \item Corollary 46 (over a local field) implies multiplication-by-$\ell$
            is an isomorphism on $\tilde E(\O_K)$. Hence
            \begin{equation*}
                E(K)[\ell^n]
                    \cong(E(K)/E_1(K))[\ell^n]
                    \cong\tilde E(\F_{p^k})[\ell^n]
            \end{equation*}
            by Theorem 36.

        \item $E$ has good reduction over every $F/K$ with reduced curve
            $\tilde E$ (as the valuation of $\Delta_E$ is 0). In particular,
            taking $F=K(E[\ell^n])$ gives
            $\tilde E(\closure\F_{p^k})\supseteq\Z/\ell^n\times\Z/\ell^n$, and
            conversely if
            $\tilde E(\F_{p^{km}})[\ell]\supseteq(\Z/\ell\Z)^{\times3}$ then so
            does $E(F)[\ell]$ by (i), which is impossible.

        \item Follows from (ii).

        \item Pick $m$ such that
            $\tilde E(\F_{p^{km}})[\ell^n]=\Z/\ell^n\Z\times\Z/\ell^n\Z$. Let
            $F/K$ be the unique degree $m$ unramified extension of $K$. Then
            $E(F)[\ell^n]=\Z/\ell^n\Z\times\Z/\ell^n\Z$ by (i).
    \end{enumerate}
\end{proof}

\begin{theorem}
    Let $E/\F_q$ be an elliptic curve over a finite field, and $\ell\nmid q$ a
    prime. Then the Frobenius automorphism (i.e. $t\mapsto t^q$ in
    $\Gal(\closure\F_q/\F_q)$) acts on $T_\ell E$ by a $2\times2$ matrix
    $M\in\GL_2(\Z_\ell)$ which is diagonalizable in $\GL_2(\closure\Q_\ell)$
    and has characteristic polynomial $P(E,T)=T^2-aT+q$ where
    $-a=\#\tilde E(\F_q)-1-q$.
\end{theorem}
...
\begin{example}
    Let $E:y^2=x^3-x+1$ over $\F_3$. Then
    \begin{equation*}
        E(\F_3) = \{\O,(1,1),(1,-1),(0,1),(0,-1),(-1,1),(-1,-1)\} \cong C_7,
    \end{equation*}
    so $-a=7-1-3=3$, and $P(E,T)=T^2+3T+3=(T-\alpha)(T-\beta)$ with
    \begin{align*}
        \alpha &= -\frac{3+\sqrt{-3}}{2}, & \beta &= -\frac{3-\sqrt{-3}}{2}.
    \end{align*}
    We do a sanity check: this says the characteristic polynomial of the
    Frobenius map $\varphi$ on $T_2E$ is $T^2+3T+3$, reducing to $T^2+T+1$ on
    $E[2]$. In other words, $\varphi$ acts on $E(\closure\F_3)[2]$ with order 3.
    Indeed $x^3-x+1$ is irreducible over $\F_3$, so $\varphi$ cyclically
    permutes the 2-torsion points by Galois theory. From the characteristic
    polynomial we can also work out
    \begin{itemize}
        \item The order of $\varphi$ on $E(\closure\F_3)[7]$.
        \item The number of points over $\F_9,\F_{27}$, e.t.c.
    \end{itemize}
\end{example}

\begin{proposition}[Recall from Galois theory]
    If $F/K$ is a Galois extension (possibly infinite) with $K$ a
    non-Archimedean local field (e.g. $\Q_p$), say with residue fields
    $\F_K,\F_F$, then
    \begin{itemize}
        \item There is a maximal unramified extension $L/K$ contained in $F$,
            with $\F_F=\F_L$. When $F=\closure K$ it is denote $L=K^\nr$.

        \item $L/K$ is Galois with a (pro-)cyclic Galois group generated by an
            element called the Frobenius element, which acts as Frobenius on
            $\F_L$ over $\F_K$.

        \item $F/L$ is totally ramified, with $\Gal(F/L)=I_{F/K}\le\Gal(F/K)$
            the inertia group. The elements of $I_{F/K}$ are precisely those
            that act trivially on $\F_F$.
    \end{itemize}
    \begin{equation*}
        \begin{tikzcd}
            &F \ar[d,dash,"\text{totally ramified}", "I_{F/K}"'] \\
            &L \ar[d,dash,"\text{unramified}", "\langle\Frob\rangle"'] \\
            &K
        \end{tikzcd}
    \end{equation*}
\end{proposition}

\begin{theorem}[Criterion of N\'eron--Ogg--Shafarevich]
    Let $E$ be an elliptic curve over a non-Archimedean local field $K$, and
    $\ell\nmid\#\F_K$ a prime.
    \begin{enumerate}[label=(\roman*)]
        \item $E$ has good reduction iff $I_{\closure K/K}$ acts trivially on
            $T_\ell E$.

        \item $E$ has multiplicative reduction iff $I_{\closure K/K}$ fixes a
            subgroup isomorphic to $\Z_\ell$ in $T_\ell E$.

        \item $E$ has additive reduction iff $I_{\closure K/K}$ fixes no
            elements of $T_\ell E$.
    \end{enumerate}
\end{theorem}

\begin{proof}
    \begin{enumerate}[label=(\roman*)]
        \item ($\implies$): By Lemma 55(iv) the extension $K(E[\ell^n])/K$ is
            unramified, so $I_{\closure K/K}$ acts trivially on $E[\ell^n]$ for
            all $n$.

            ($\impliedby$): Since $I_{\closure K/K}$ acts trivially on
            $E[\ell^n]$ for all $n$, we get that $F_n=K(E[\ell^n])/K$ is
            unramified for all $n$. Then $E(F_n)$ contains
            $\Z/\ell^n\Z\times\Z/\ell^n\Z$. Since the Tamagawa number
            $\#E(F)/E_0(F)$ is finite, and bounded for unramified extensions
            $F/K$ by Theorem 35, we get that $E_0(F_n)$ contains
            $\Z/\ell\Z\times\Z/\ell\Z$ for $n\gg0$. Then
            $\tilde E_\ns(\F_{F_n})\cong E_0(F_n)/E_1(F_n)$ contains
            $\Z/\ell\Z\times\Z/\ell\Z$, as multiplication-by-$\ell$ is an
            isomorphism on $\tilde E(\O_{F_n})\cong E_1(F_n)$. Then
            $\tilde E_\ns(\closure\F_K)$ contains $\Z/\ell\Z\times\Z/\ell\Z$,
            while the groups $\F_K^\times$ and $(\F_K,+)$ are cyclic and
            therefore contain at most one copy of $\Z/\ell\Z$, so the reduction
            cannot be multiplicative or additive. Therefore $E$ has good
            reduction.

        \item Sketch: $\tilde E_\ns(\F_F)\cong\F_F^\times$ contains
            $\Z/\ell^n\Z$ for sufficiently large unramified $F/K$, but never
            contains $\Z/\ell^n\Z\times\Z/\ell^n\Z$. Then run a similar argument
            to (i).

        \item Follows from (i) and (ii) by exclusion.
    \end{enumerate}
\end{proof}

\begin{remark}
    If $E/K$ has multiplicative reduction and $\ell\nmid c(E/K)$, the same proof
    shows that $I_{\closure K/K}$ acts non-trivially on $E(K)[\ell]$.
\end{remark}

\begin{remark}
    If $E/K$ has good reduction, then Lemma 55(i) and Theorem 57(i) imply that
    the Frobenius element acts on $T_\ell E$ the same way as the Frobenius
    automorphism does on $T_\ell\tilde E$, so its characteristic polynomial is
    $T^2-aT+q$ where $q=\#\F_K$, $-a=\#\tilde E(\F_K)-q-1$.
\end{remark}

\begin{theorem}
    Let $E/K$ be an elliptic curve over a non-Archimedean local field, and
    $\ell\nmid\#\F_K$ a prime. If $g\in I_{\closure K/K}$ acts on $T_\ell E$ by
    $M\in\GL_2(\Z_\ell)$, then
    \begin{enumerate}[label=(\roman*)]
        \item $\det M=1$.
        \item The characteristic polynomial of $M$ is independent of $\ell$, and
            hence has integer coefficients.
    \end{enumerate}
\end{theorem}

\begin{proof}
    (i) can be proved using the Weil pairing. (ii) requires some work.
\end{proof}

\begin{example}
    Take $E:y^2+y=x^3-x^2$, and let $F=\Q(E[5])$. Since $E(\Q)$ has an element
    of order 5, we have
    \begin{equation*}
        \Gal(F/\Q) \le \left\{\begin{pmatrix}
                1 & * \\ 0 & *
        \end{pmatrix}\right\} \le \GL_2(\F_5).
    \end{equation*}
    Now $E$ has multiplicative reduction at 11, and $\ord_11\Delta_E=1$, so the
    Tamagawa number $c_11=1$. By the above remark the inertia group of $\Q_11$
    acts non-trivially on $E(\closure\Q)[5]$, and by Theorem 58 it has
    determinant 1. Hence it acts via matrices of the form
    $\begin{pmatrix}1&*\\0&1\end{pmatrix}$ with $*\ne0$. Moreover the
    characteristic polynomial of Frobenius at 2 is of the form $T^2-aT+2$, so
    Frobenius has determinant 2 and acts via a matrix of the form
    $\begin{pmatrix}1&*\\0&2\end{pmatrix}$ on $E(\closure\Q)[5]$. Hence
    \begin{equation*}
        \Gal(F/\Q)=\left\{\begin{pmatrix}
                1 & * \\ 0 & *
        \end{pmatrix}\right\} \cong C_5\rtimes C_4.
    \end{equation*}
\end{example}
...
\begin{enumerate}
    \item[+1.] Let $E/\F_7$ be the elliptic curve given by $y^2+y=x^3-x^2$.
        \begin{enumerate}[label=(\roman*)]
            \item Find the eigenvalues of the action of the Frobenius
                automorphism on $T_l(E)$ for $l\ne7$.
            \item Determine the number of solutions to the equation over
                $\F_{7^{11}}$.
        \end{enumerate}

    \item[+2.] Let $E/\Q_5$ be the elliptic curve given by $y^2=x^3-25$.
        \begin{enumerate}[label=(\roman*)]
            \item Show that $E$ has additive reduction over $\Q_5$, but good
                reduction over $K=\Q_5(\sqrt[3]5)$, and deduce that the inertia
                group acts on $T_l(E)$ through a group $\langle g\rangle$ of
                order 3.

            \item Determine the eigenvalues of the action of the Frobenius
                element $\phi\in\Gal(K^\nr/K)$ on $T_l(E)$ for $l\ne5$.

            \item Using the fact that $\phi g\phi^{-1}=g^{-1}$, show that with
                respect to a suitable $\closure\Q_l$-basis, the matrices for the
                action of $g$ and $\phi$ on $T_l(E)$ are given by
                \begin{equation*}
                    \begin{pmatrix}
                        \zeta_3 & 0 \\ 0 & \zeta_3^{-1}
                    ...
                    \end{pmatrix}
                \end{equation*}
                where $l\ne5$ is prime, and $\zeta_3$ and $\sqrt{-5}$ a
                primitive cube root of 1 and a square root of $-5$ in
                $\closure\Q_l$.
        \end{enumerate}

    \item[3.] Given that there is an elliptic curve $E$ over $\Q_2$ with
        $[\Q_2(E[3]):\Q_2]=48$, prove that $\SL_2(\F_3)$ is a subgroup of
        $\GL_2(\F_p)$ for every prime $p\ge3$.

    \item[4.] Solve the inverse Galois problem for the groups $G=\GL_2(\F_p)$.

        (\textit{There's a stnadard result in group theory that says that if
        $H\le\GL_2(\F_p)$ contains an element of order $p$ and preserves no
        1-dimensional subspace of $\F_p^2$, then $H$ contains $\SL_2(\F_p)$.})

    \item[!5.] Let $E/\Q$ be an elliptic curve. Show that either
        $\Gal(\Q(E[p])/\Q)$ is soluble for every prime $p$, or
        $\Gal(\Q(E[p])/\Q)=\GL_2(\F_p)$ for every prime $p>37$.
\end{enumerate}
