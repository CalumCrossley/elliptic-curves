...
\begin{enumerate}
    \item[+1.] Let $E/\Q$ be an elliptic curve over $\Q$ given by
        ...
        \begin{proof}[Solution]
            ...
            for primes $p\ge2$, so taking the logarithm of a prime factorization
            we see that
            \begin{equation*}
                N\le 1 + \log\bigl(
                    2|\alpha-\beta||\beta-\gamma||\alpha-\gamma|\bigr).
            \end{equation*}
            Hence
            \begin{equation*}
                \rk(E/\Q)
    ...
    \item[+2.] Suppose $A$ is an abelian group with $A/2A$ finite that admits a
        ...
        \begin{proof}[Solution]
            Fix a complete set $S\subseteq A$ of representatives for the
            elements of $A/2A$. Given $x\in A$, we have a unique $a_1\in S$ with
            $x-a_1=2y$ for some $y\in A$. Continuing inductively, we get a
            unique sequence $a_1,a_2,\ldots\in S$ such that
            $x-a_1-2a_2-\cdots-2^na_n=2^{n+1}y$ for some $y\in A$ for each $n$.
        \end{proof}
    ...
    \item[3.] Let $E/K$ be an elliptic curve over a number field, such that
        ...
        \begin{proof}[Solution]
            ...
            the maps $f_{P_i}$, since $\sigma(P_i)=P_i$ we get
            \begin{equation*}
                2f_{P_i}(\sigma) = \sigma(P_i)\ominus P_i = 0
            \end{equation*}
            and
            \begin{align*}
                f_{P_i}(\sigma\tau)
                    &= \sigma\tau(Q_i)\ominus Q_i \\
                    &= (\sigma\tau(Q_i)\ominus\tau(Q_i)) % TODO: wrong
                        \oplus (\tau(Q_i)\ominus Q_i) \\
                    &= f_{\tau(P_i)}(\sigma) \oplus f_{P_i}(\tau) \\
                    &= f_{P_i}(\sigma) \oplus f_{P_i}(\tau),
            \end{align*}
            so $f_{P_i}(\sigma)\in E(F)[2]=E(K)[2]$ since
            $E(K)[2]\cong C_2\times C_2$, and $f_{P_i}$ is a homomorphism.
            Moreover
            \begin{align*}
            ...
            \end{align*}
            so this gives a homomorphism $E(K)/2E(K)\to\Hom(G,E(K)[2])$, and if
            $f_{P_i}=0$ then $\sigma(Q_i)=Q_i$ for all $\sigma\in G$, and hence
            $Q_i\in E(K)$, so $P_i\in 2E(K)$. Therefore it is injective.
