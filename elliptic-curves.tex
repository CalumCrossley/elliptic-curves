...
\begin{enumerate}
    \item[+1.] Let $E/\Q$ be an elliptic curve given by $E:y^2=x^3+Ax+B$, and
        ...
        \begin{proof}[Solution]
            ...
            \end{equation*}
            where the error term $O(1)$ only depends on $E$. Then using the
            ...
            \begin{equation*}
                h_\Q(x_n)
                    ...
                    = n^2\hat h(P) + O(1),
            \end{equation*}
            so $a=\hat h(P)$ suffices. The error term $O(1)$ here is just that
            from $h_\Q(x_n)=\hat h(nP)+O(1)$, which may depend on $E$ but does
            not depend on the point in question.
        \end{proof}
