...
\begin{enumerate}
    ...
    \item[+2.] Suppose $A$ is an abelian group with $A/2A$ finite that admits a
        ...
        \begin{proof}[Solution]
            Let $C\in\R$ be such that $|h(x+y)+h(x-y)-2h(x)-2h(y)|\le C$ for
            all $x,y\in A$. Fix a complete set $S\subseteq A$ of representatives
            for the elements of $A/2A$. Given $x\in A$, we have some $a_1\in S$
            with $x+a_1=2y$ for some $y\in A$, and continuing inductively we get
            $a_1,a_2,\ldots\in S$ sastifying $x+a_1+2a_2+\cdots+2^na_n=2^{n+1}y$
            ...
            \begin{align*}
                h(2^{n+1}y)
                    &= h(x+a_1+2a_2+\cdots+2^na_n) \\
                    &\le 2h(x) + 2h(a_1+2a_2+\cdots+2^na_n) + C \\
                    &\le 2h(x) + 2^2h(a_1) + \cdots + 2^{n+1}h(2^na_n)
                        + (1+2+2^2+\cdots+2^n)C \\
                    &\le % TODO
            \end{align*}
        \end{proof}
